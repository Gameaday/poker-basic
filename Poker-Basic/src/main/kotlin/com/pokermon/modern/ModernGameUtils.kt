...
class ModernGameManager {
     ...
     */
    suspend fun startGame(players: List<Player>, mode: GameMode): GameResult<GameState> = 
     ...
     */
    fun placeBet(playerId: String, amount: Int): GameResult<GameState> {
        val currentState = _gameState.value
        val player = currentState.players.find { it.name == playerId }
     ...
     */
    suspend fun advancePhase(): GameResult<GameState> = withContext(Dispatchers.Default) {
        val currentState = _gameState.value
     ...
     */
    fun observeBettingEvents(): Flow<GameEvent.BetPlaced> = 
        gameEvents.filterIsInstance<GameEvent.BetPlaced>()
