...
class MonsterOpponentManager {
    ...
    companion object {
        ...
        private val beginnerOpponents = listOf(
            "PixelPup", "ByteBird", "CodeCat", "DataDog"
        )
        
        private val intermediateOpponents = listOf(
            "FireFox.exe", "AquaApp", "TechTurtle", "CloudCrawler"
        )
        
        private val advancedOpponents = listOf(
            "NeuralNinja", "QuantumQuokka", "CyberShark", "RoboRaven"
        )
        
        private val expertOpponents = listOf(
            "MegaMind.AI", "DragonDrive", "PhoenixProtocol"
        )
        
        private val legendaryOpponents = listOf(
            "The Compiler", "Daemon.exe", "The Algorithm"
        )
    }
     ...
     */
    fun generateOpponents(opponentCount: Int, playerSkillLevel: Int = 0): List<MonsterOpponent> {
    ...
    private fun calculateAggressiveness(monster: Monster): Float {
        val baseAggressiveness = when (monster.effectType) {
    ...
    private fun calculateBluffFrequency(monster: Monster): Float {
        ...
        val baseBluffing = when (monster.effectType) {
