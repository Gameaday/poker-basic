...
object Main {
    ...
    private fun initializePlayers(
        ...
        for (i in 0..opponentCount) {
            ...
            if (player is AIPlayer) {
                println(
                    "${player.name} has ${player.personality.displayName} personality " +
     ...
     */
    private fun reinitializePlayers(
        players: Array<Player?>,
        deck: IntArray,
    ) {
     ...
     */
    private fun readIntWithDefault(
        default: Int,
    ...
    private fun conductBettingRound(
        ...
        players.forEach { player ->
            if (player.chips > 0) {
                val bet =
                    if (player.name.contains("CPU") || player.name in POSSIBLE_NAMES) {
                        // AI betting logic
    ...
    fun cardMultiples(hand: IntArray): IntArray {
        val rankCounts =
            hand.groupBy { CardUtils.cardRank(it) }
