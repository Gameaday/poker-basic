 ...
 */
fun Player.isActive(): Boolean = !this.fold && this.chips > 0

/**
 * Gets the player's status as a descriptive string.
 * @return formatted status string
 */
fun Player.getStatus(): String =
    when {
        this.fold -> "Folded"
 ...
 */
fun Player.getHandDisplay(): String =
    buildString {
 ...
 */
fun Array<Player>.getActivePlayers(): List<Player> = this.filter { it.isActive() }

/**
 * Gets the player with the highest hand value.
 * @return player with best hand, or null if none found
 */
fun Array<Player>.getBestHand(): Player? = this.filter { !it.fold }.maxByOrNull { it.handValue }

/**
 * Gets the total pot from all player bets.
 * @return sum of all player bets
 */
fun Array<Player>.getTotalPot(): Int = this.sumOf { it.getBetSafely() }

/**
 * Checks if only one player remains active.
 * @return true if exactly one player is still active
 */
fun Array<Player>.hasWinner(): Boolean = this.count { it.isActive() } == 1
 ...
 */
fun String.toIntSafely(defaultValue: Int = 0): Int = this.toIntOrNull() ?: defaultValue
 ...
 */
fun GameMode.getRecommendedAICount(): Int =
    when (this) {
        GameMode.CLASSIC -> 3
 ...
 */
fun GamePhase.allowsInteraction(): Boolean = this.allowsBetting() || this.allowsCardExchange() || this.allowsRoundProgression()

/**
 * Gets the estimated duration for this phase in seconds.
 * @return estimated time in seconds
 */
fun GamePhase.getEstimatedDuration(): Int =
    when (this) {
