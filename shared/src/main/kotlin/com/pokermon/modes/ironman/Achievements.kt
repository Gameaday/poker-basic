...
object IronmanAchievements {
    ...
    fun updateSessionOnRound(
        ...
        return when (result.outcome) {
            RoundOutcome.VICTORY -> session.copy(
            ...
            RoundOutcome.DEFEAT -> session.copy(
                currentStreak = 0,
            ...
            RoundOutcome.DRAW -> session.copy(
                riskLevel = riskLevel
            )
        }
    }
    ...
    fun updateSessionOnGacha(
        ...
        val legendaryPulls = if (monster.rarity == Monster.Rarity.LEGENDARY) {
            session.legendaryPulls + 1
        } else {
            session.legendaryPulls
        }
        
        val consecutiveLegendaryPulls = if (monster.rarity == Monster.Rarity.LEGENDARY) {
            session.consecutiveLegendaryPulls + 1
        } else {
            0
        }
        
        val pitySaves = if (isPityBreaker && monster.rarity == Monster.Rarity.LEGENDARY) {
            session.pitySaves + 1
        } else {
            session.pitySaves
        }
        ...
        return session.copy(
            ...
            maxConsecutiveLegendaryPulls = maxOf(
     ...
     */
    fun updateSessionOnRevival(session: IronmanSession, method: String): IronmanSession {
     ...
     */
    fun updateSessionOnLevelUp(session: IronmanSession, newLevel: Int): IronmanSession {
        return session.copy(
