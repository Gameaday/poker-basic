...
class Player {
    ...
    public void convertHand() {
        convertedHand = simple_main.convertHand(hand);
    }

    // converts pairs to a reader friendly format
    public void convertHand2() {
        convertedHand2 = simple_main.convertHand2(handMultiples);
    }

    // find card multiples in hand
    public void findMultiples() {
        handMultiples = simple_main.handMultiples(hand);
    }

    // checks if hand is a Straight
    public void isStraight() {
        Straight = simple_main.isStraight(hand);
    }

    // checks if hand is a Straight
    public void isAceStraight() {
        aceStraight = simple_main.isAceStraight(hand);
    }

    // checks if hand is Flush
    public void isFlush() {
        Straight = simple_main.isFlush(hand);
    }

    // checks if hand is a Straight Flush
    public void isStraightFlush() {
        straightFlush = simple_main.isStraightFlush(hand);
    }

    // checks if hand is a Royal Flush
    public void isRoyalFlush() {
        royalFlush = simple_main.isRoyalFlush(hand);
    }

    // checks if Two of a kind
    public void is2Kind() {
        twoKind = simple_main.is2Kind(handMultiples);
    }

    // checks if Two pairs
    public void is2Pair() {
        twoPair = simple_main.is2Pair(handMultiples);
    }

    // checks if Three of a kind
    public void is3Kind() {
        threeKind = simple_main.is3Kind(handMultiples);
    }

    // checks if Full House
    public void isFullHouse() {
        fullHouse = simple_main.isFullHouse(handMultiples);
    }

    // checks if Four of a kind
    public void is4Kind() {
        fourKind = simple_main.is4Kind(handMultiples);
    }
    ...
    public void handValue() {
        handValue = simple_main.handValue(hand);
    }
    ...
    public void setupPlayer(String playerName, int chips, int[] Deck) {
        ...
        resetFold();
        updateHand(simple_main.newHand(Deck));
        convertHand();
        printPlayer();
        printHandStats();
        reportPlayer();
    ...
    // only prints the highest value play that the hand contains
    // this is a bit inellegent, but it works
...
public class simple_main {
    ...
    static void recursiveBet(Player[] Player, int i, int pot, int bet) {
        for (int j = i + 1; j < Player.length; j++) {
            if (j == 0 && Player[j].fold == false) {
                Player USER = Player[j];
                pot += USER.bet(bet);
                USER.lastBet();
            }
            if (j == 1 && Player[j].fold == false) {
                Player CPU1 = Player[j];
                pot += CPU1.bet(bet);
                CPU1.lastBet();
            }
            if (j == 2 && Player[j].fold == false) {
                Player CPU2 = Player[j];
                pot += CPU2.bet(bet);
                CPU2.lastBet();
            }
            if (j == 3 && Player[j].fold == false) {
                Player CPU3 = Player[j];
                pot += CPU3.bet(bet);
                CPU3.lastBet();
            }
        }
    ...
    public static boolean isRoyalFlush(int[] hand) {
        boolean isRoyalFlush, aceStraight, flush;
        flush = isFlush(hand);
